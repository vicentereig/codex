...
use super::failure_injection_recovery_outbox_matrix_support::*;
...
#[tokio::test]
async fn publication_wrappers_exhaust_counted_crash_recovery_matrix() -> anyhow::Result<()> {
    for case in OUTBOX_CASES {
        run_outbox(case).await?;
    }
    Ok(())
}

async fn run_outbox(case: OutboxCase) -> anyhow::Result<()> {
...
fn assert_outbox_success(case: OutboxCase, output: &OutboxOutput) {
    let valid = match (case, output) {
        (
            OutboxCase::Claim,
            OutboxOutput::Claim(ClaimDegradationPublicationsOutcome::Claimed(values)),
        ) => values.len() == 2,
        (
            OutboxCase::Materialized,
            OutboxOutput::Resolve(ResolveDegradationPublicationOutcome::Applied(
        ...
        | (
            OutboxCase::Retry,
            OutboxOutput::Resolve(ResolveDegradationPublicationOutcome::Applied(
                DegradationPublicationStatus::Pending,
            )),
        )
        | (
            OutboxCase::Poisoned | OutboxCase::RetryExhausted,
            OutboxOutput::Resolve(ResolveDegradationPublicationOutcome::Applied(
